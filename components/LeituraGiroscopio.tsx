...
export default function App() {
  ...
  return (
    <View style={styles.container}>
      ...
      */}
      <Text style={styles.text}>x: {Math.abs(x) < 0.05 ? '0.0' : x.toFixed(1)}</Text>
      <Text style={styles.text}>y: {Math.abs(y) < 0.05 ? '0.0' : y.toFixed(1)}</Text>
      <Text style={styles.text}>z: {Math.abs(z) < 0.05 ? '0.0' : z.toFixed(1)}</Text>
    </View>
  );
}
